...
void decrypt(const char* __firststring, const char* __secondstring, int64_t __n, int64_t __d);
...
uint64_t modulo(ull base, ull e, ull mod) {
	ull a = 1;
	ull b = base;
	while (e > 0) {
		if (e % 2 == 1)
...
bool fermat(ull m, int iterations) {
	if (m == 1) {
		return false;
	}
...
std::pair<std::string, std::string> generate_key() {
	ull p = 0, q = 0, e = 0, f = 0, d = 0, n = 0;

	p = generate_big_prime_number();
	q = generate_big_prime_number();
